...
| `rolePinning` | **Optional phase→model map — soft preferences with fallback semantics.** The intent router gets first shot at picking the best model for each phase (informed by phase context, model descriptions, and cost). The pin fires only if the intent tier produces no result (classifier error, no candidates, hallucinated model ID). The classifier sees exactly the unique models declared in the pin values — not every model from the allowed providers — so it chooses from a focused candidate list. Keys are `LedgerPhase` strings (`discuss`, `dispatch`, `execute`, `verify`, `reflect`, `compress`, `state_update`, `consult`). When comparable models are pinned to overlapping roles (e.g. Opus and GPT-5.4 both strong at planning), the router can choose between them based on task complexity and cost. If a pinned model ID is missing or disabled, routing falls through to rules. |
